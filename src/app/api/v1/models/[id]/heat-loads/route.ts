...
import { thermalModels, heatLoads } from '@/lib/db/schema';
...
export async function GET(request: Request, { params }: RouteParams): Promise<NextResponse> {
  try {
    ...
    const rows = await db.select().from(heatLoads).where(eq(heatLoads.modelId, id));

    return NextResponse.json({ data: rows });
  } catch (error) {
    console.error('GET /api/v1/models/[id]/heat-loads error:', error);
...
export async function POST(request: Request, { params }: RouteParams): Promise<NextResponse> {
  try {
    ...
    for (const item of items) {
      const parsed = createHeatLoadSchema.safeParse(item);
      if (!parsed.success) return validationErrorResponse(parsed.error);
      results.push({ modelId: id, ...parsed.data });
    }

    const created = await db.insert(heatLoads).values(results).returning();

    return NextResponse.json({ data: Array.isArray(body) ? created : created[0] }, { status: 201 });
  } catch (error) {
    console.error('POST /api/v1/models/[id]/heat-loads error:', error);
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 });
  }
}
